...
function testGame1Execution(type='NE') {
    ...
    for (let i = 0; i < bestSetting.length; i++) {
        ...
        for (let j = 0; j < maxRounds; j++) {
            log(resultArray[i][bs.populationIndex][bs.generationCountIndex][bs.mutationIndex][bs.selectionFunctionIndex][bs.replacementFunctionIndex][j], '|', resultArray[4][j]);
    ...
    fs.writeFile("results/game2.txt", output, function(err) {
        if(err) {
            return log(err);
        }
    }); 
